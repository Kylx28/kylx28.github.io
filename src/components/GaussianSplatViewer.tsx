    ...
    return (
      <div className={`group relative aspect-[16/10] overflow-hidden border border-line bg-paper ${className}`}>
        ...
        <div className="absolute inset-x-0 bottom-0 flex flex-col items-start justify-between gap-4 border-t border-white/70 bg-white/90 p-4 backdrop-blur-sm sm:flex-row sm:items-center">
          <div>
            <p className="flex items-center gap-2 text-sm font-medium"><AlertTriangle size={15} />3D preview unavailable</p>
            <p className="mt-1 text-xs text-muted">The static preview is still available.</p>
          </div>
          <button onClick={() => setFailed(false)} className="border border-ink bg-ink px-4 py-2.5 font-mono text-[10px] uppercase tracking-[0.12em] text-white transition-colors hover:bg-white hover:text-ink">
            Try again
          </button>
        </div>
      </div>
